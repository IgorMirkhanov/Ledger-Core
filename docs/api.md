...
| Идемпотентность | Все `POST` **требуют** `Idempotency-Key` (1..128 символов, рекомендуется UUID). Нет ключа: `400 IDEMPOTENCY_KEY_MISSING`. Ключ хранится в пространстве имён вызывающего, поэтому одинаковый ключ у двух пользователей — это две разные операции |
