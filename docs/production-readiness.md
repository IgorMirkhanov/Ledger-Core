...
| Сторонние GitHub Actions по тегам, а не по SHA (кроме trivy-action: уже по SHA после перевыпуска его тегов) | подмена тега | закрепить SHA остальных (Dependabot будет обновлять) |
