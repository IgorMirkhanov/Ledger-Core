...
Каждые `RECOVERY_INTERVAL` (1s): `ClaimPending(now, 50)` → для каждого id `Advance(id)`
с ограничением параллелизма (semaphore на 10). Несколько реплик transfers безопасны
благодаря `FOR UPDATE SKIP LOCKED` + версиям.
